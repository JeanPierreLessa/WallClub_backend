...
# Database (MySQL compartilhado com app principal)
# Tenta AWS Secrets Manager primeiro, depois fallback para ENV vars (desenvolvimento local)
...
try:
    ...
    print(f"[WARNING] Não foi possível acessar AWS Secrets Manager: {e}")
    db_config = None

if not db_config:
    # Fallback para variáveis de ambiente (desenvolvimento local)
    print("[INFO] Usando configurações de banco de dados do .env")
    DATABASES = {
        'default': {
            'ENGINE': 'django.db.backends.mysql',
            'NAME': os.environ.get('DB_NAME', 'wallclub'),
...
# MaxMind minFraud (Semana 9) - Lê do AWS Secrets Manager ou ENV vars
try:
    if db_config:  # Só tenta se tiver config_manager funcionando
        ...
        raise Exception("Config manager não disponível")
except Exception as e:
    print(f"[WARNING] MaxMind config não disponível via AWS: {e}")
