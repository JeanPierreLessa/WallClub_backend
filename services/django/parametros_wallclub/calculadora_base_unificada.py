...
Processa transações de checkout (Wallet) e POS
IMPORTANTE: Não busca dados diretamente - tudo via parâmetros
"""
...
class CalculadoraBaseUnificada:
    ...
    def calcular_valores_primarios(self, dados_linha, tabela: str, info_loja: Dict[str, Any], info_canal: Dict[str, Any]):
        """
        Calcula os valores primários baseados nos dados da transação.

        Args:
            dados_linha: Dict com dados da transação
            tabela: 'transactiondata_pos' (compatibilidade)
        ...
        try:
            ...
            registrar_log('parametros_wallclub', f"Iniciando cálculo unificado ID: {log_id}")
            valores = {}
