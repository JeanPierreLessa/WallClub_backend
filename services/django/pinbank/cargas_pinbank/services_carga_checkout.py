"""
Serviço para carga da Base de Gestão - Checkout
...
class CargaCheckoutService:
    """
    Serviço para carga da base de gestão - Checkout
    Processa transações que estão na checkout_transactions
    """
    ...
    def carregar_valores_primarios(self, limite: int = None, nsu: str = None) -> int:
        """
        Rotina principal de carga de variáveis primárias para Checkout
        Processa registros com lido = 0
        """
        registrar_log('pinbank.cargas_pinbank', "Iniciando carga de valores primários Checkout")

        # Query principal - busca registros não lidos de Checkout
        ...
        with connection.cursor() as cursor:
            ...
            while True:
                ...
                if len(lote_atual) >= BATCH_SIZE:
                    registrar_log('pinbank.cargas_pinbank', f"Processando lote Checkout {numero_lote}: {len(lote_atual)} registros")
                    ...
                    with transaction.atomic():
                        for row_lote in lote_atual:
                            ...
                            try:
                                # Montar info_loja e info_canal a partir dos dados da query (Checkout não tem em transactiondata)
                                linha['info_loja'] = {
                                ...
                                registrar_log('pinbank.cargas_pinbank', f"Erro crítico (Checkout): ID={linha['id']}, Erro: {str(e)}", nivel='ERROR')
                    ...
                    registrar_log('pinbank.cargas_pinbank', f"Lote Checkout {numero_lote} commitado com sucesso ({len(lote_atual)} registros processados)")
                    lote_atual = []
                    numero_lote += 1

            # Processar último lote se houver registros restantes
            if lote_atual:
                registrar_log('pinbank.cargas_pinbank', f"Processando último lote Checkout {numero_lote}: {len(lote_atual)} registros")
                ...
                registrar_log('pinbank.cargas_pinbank', f"Último lote Checkout {numero_lote} commitado com sucesso ({len(lote_atual)} registros processados)")

        registrar_log('pinbank.cargas_pinbank', f"Carga de valores primários Checkout finalizada. Registros processados: {registros_processados}")
    ...
    def _inserir_valores_base_gestao(self, valores: Dict[int, Any], cliente_id: int = None) -> bool:
        ...
        try:
            ...
            # CargaCheckoutService SEMPRE é Checkout
            campos_mapeados['banco'] = 'PIN'
            campos_mapeados['tipo_operacao'] = 'Checkout'
            ...
            registrar_log('pinbank.cargas_pinbank', f"Erro ao inserir valores na base de gestão Checkout: {str(e)}", nivel='ERROR')
