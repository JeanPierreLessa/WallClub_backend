...
class SaldoService:
    ...
    @staticmethod
    def consultar_saldo_cashback(cpf: str, terminal: str, valor_compra: Decimal) -> Dict[str, Any]:
        """
        Consulta APENAS saldo de cashback do cliente sem validar senha.
        Verifica se cliente tem cashback disponível e calcula valor máximo permitido para uso.
        ...
        Returns:
            dict com sucesso, mensagem, tem_saldo, cashback_disponivel e valor_maximo_permitido
        """
        ...
        if not dados_terminal or 'canal_id' not in dados_terminal:
            registrar_log('posp2', f'❌ [CONSULTA CASHBACK] Terminal não encontrado: terminal={terminal}')
            return {
                'sucesso': False,
        ...
        try:
            ...
            if not resultado.get('sucesso'):
                return {
                    'sucesso': True,
                    'mensagem': 'Cliente não possui cashback',
                    'tem_saldo': False
                }

            # Usar APENAS cashback_disponivel
            cashback_disponivel = Decimal(resultado.get('cashback_disponivel', '0.00'))
            tem_saldo = cashback_disponivel > 0

            registrar_log('posp2',
                f'✅ [CONSULTA CASHBACK] Cashback consultado: cpf={cpf[:3]}***, cashback={cashback_disponivel}')

            # Preparar resposta base
            resposta = {
                'sucesso': True,
                'mensagem': 'Cliente possui cashback disponível' if tem_saldo else 'Cliente não possui cashback',
                'tem_saldo': tem_saldo,
            ...
            if tem_saldo and loja_id:
                try:
                    ...
                    if resultado_calculo.get('sucesso'):
                        ...
                        registrar_log('posp2',
                            f'💰 [CONSULTA CASHBACK] Valor máximo calculado: '
                            f'compra={valor_compra}, cashback={cashback_disponivel}, '
                            f'permitido={valor_permitido:.2f}, validation_token_gerado=True')
                    else:
                        registrar_log('posp2',
                            f'⚠️ [CONSULTA CASHBACK] Erro ao calcular valor máximo: {resultado_calculo.get("mensagem")}')
                except Exception as e:
                    registrar_log('posp2',
                        f'⚠️ [CONSULTA CASHBACK] Exceção ao calcular máximo: {str(e)}',
                        nivel='WARNING')
            ...
            registrar_log('posp2',
                f'❌ [CONSULTA CASHBACK] Erro ao consultar: {str(e)}',
                nivel='ERROR')
            return {
                'sucesso': False,
                'mensagem': 'Erro ao consultar cashback',
                'tem_saldo': False
            }
