...
class RecorrenciaTokenService:
    ...
    def processar_cadastro_cartao(
        ...
        1. Valida token
        2. Tokeniza o cartão
        3. Vincula cartão à recorrência
        ...
        try:
            ...
            registrar_log(
                'checkout.recorrencia',
                f"💳 Iniciando cadastro de cartão para recorrência {recorrencia.id}"
            )
            ...
            registrar_log(
                'checkout.recorrencia',
                f"🔑 Tokenizando cartão para uso futuro"
            )
            ...
            if not resultado_token.get('sucesso'):
                registrar_log(
                    'checkout.recorrencia',
                    f"❌ Erro ao tokenizar cartão: {resultado_token.get('mensagem')}",
                    nivel='ERROR'
                )
                return {
                    'sucesso': False,
            ...
            # ========== VINCULAR CARTÃO À RECORRÊNCIA ==========
            registrar_log(
                'checkout.recorrencia',
                f"🔗 Vinculando cartão à recorrência"
            )
