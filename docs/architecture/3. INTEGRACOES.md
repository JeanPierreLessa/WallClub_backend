# INTEGRAÇÕES - WALLCLUB ECOSYSTEM

**Versão:** 3.1  
**Data:** 02/11/2025  
**Status:** APIs internas + externas operacionais + wallclub_core extraído  
**Mudanças:** Monorepo unificado, 26 APIs REST internas, wallclub_core compartilhado
...
1. [APIs Internas - Overview](#apis-internas---overview) ⭐ NOVO
2. [Conta Digital APIs](#conta-digital-apis) ⭐ NOVO
3. [Checkout Recorrências APIs](#checkout-recorrências-apis) ⭐ NOVO
4. [Ofertas APIs](#ofertas-apis) ⭐ NOVO
5. [Parâmetros APIs](#parâmetros-apis) ⭐ NOVO

### Integrações Externas
6. [Pinbank](#pinbank)
7. [MaxMind minFraud](#maxmind-minfraud)
8. [Risk Engine - Autenticação Cliente](#risk-engine---autenticação-cliente)
...
**Status:** Fase 6B concluída (01/11/2025)  
**Total:** 26 endpoints REST  
**Propósito:** Comunicação entre containers desacoplados

### Características

**Autenticação:** OAuth 2.0 (scope `internal`)  
**Rate Limiting:** Desabilitado (containers confiáveis)  
**Timeout:** 5s (consulta), 10s (escrita)  
**Base URL:** `http://localhost:8000/api/internal/`
...
1. `POST /consultar-saldo/`
2. `POST /autorizar-uso/`
3. `POST /debitar-saldo/`
4. `POST /estornar-saldo/`
5. `POST /calcular-maximo/`

---

## 🔁 CHECKOUT RECORRENCIAS APIS
...
2. `POST /criar/` - Criar
3. `GET /{id}/` - Obter
4. `POST /{id}/pausar/` - Pausar
5. `POST /{id}/reativar/` - Reativar
6. `POST /{id}/cobrar/` - Cobrar
7. `PUT /{id}/atualizar/` - Atualizar
8. `DELETE /{id}/deletar/` - Cancelar
...
1. `POST /configuracoes/loja/`
2. `POST /configuracoes/contar/`
3. `POST /configuracoes/ultima/`
4. `POST /loja/modalidades/`
5. `POST /planos/`
6. `GET /importacoes/`
7. `GET /importacoes/{id}/`
...
**Ambiente:** Produção  
**Autenticação:** Basic Auth (credenciais AWS Secrets)  
...
```json
{
  ...
  "NumeroParcelas": 1,
  "UrlRetorno": "https://apidj.wallclub.com.br/callback/",
  "terminal": "12345678"
}
```
...
#### 3. CapturarTransacaoEncrypted ⭐ NOVO

**Uso:** Captura de transações pré-autorizadas
...
**URL Base:** `http://wallclub-prod-release300:8003`
...
**Base URL:** `http://localhost:8000` (mesmo container por enquanto)  
